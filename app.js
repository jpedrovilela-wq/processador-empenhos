...
$("arquivo").addEventListener("change", (e) => {
  ...
  $("processar").disabled = $("resetar").disabled = !arquivo;
  $("status").textContent = arquivo ? "Arquivo selecionado. Escolha uma operação." : "Selecione uma planilha para começar.";
});
$("processar").onclick = () => executar("processar");
$("resetar").onclick = () => executar("resetar");
...
function removerLinha(ws, linha) {
  const area = ref(ws);
  for (let r = linha; r < area.e.r; r++) for (let c = 0; c <= area.e.c; c++) {
    const de = XLSX.utils.encode_cell({r: r + 1, c}), para = XLSX.utils.encode_cell({r, c});
    if (ws[de]) ws[para] = clonar(ws[de]); else delete ws[para];
  }
  for (let c = 0; c <= area.e.c; c++) delete ws[XLSX.utils.encode_cell({r: area.e.r, c})];
  area.e.r--; ws["!ref"] = XLSX.utils.encode_range(area);
}
function apagarColunas(ws, inicio, quantidade) {
  const area = ref(ws);
  for (let r = 0; r <= area.e.r; r++) for (let c = inicio; c <= area.e.c - quantidade; c++) {
    const de = XLSX.utils.encode_cell({r, c: c + quantidade}), para = XLSX.utils.encode_cell({r, c});
    if (ws[de]) ws[para] = clonar(ws[de]); else delete ws[para];
  }
  for (let r = 0; r <= area.e.r; r++) for (let c = area.e.c - quantidade + 1; c <= area.e.c; c++) delete ws[XLSX.utils.encode_cell({r, c})];
  area.e.c -= quantidade; ws["!ref"] = XLSX.utils.encode_range(area);
}
...
function processar(wb) {
  ...
  [[14,"Saldo utilizado NE"],[15,"Saldo remanescente NE"],[16,"Controle Macro"]].forEach(([c,t]) => { if (!valor(corh,4,c)) set(corh,4,c,t); });
  ...
  return {inseridas,usados,nao};
}
function resetar(wb) { if(!wb.SheetNames.includes(CORH))throw new Error("A aba 'ajustada CORH' não foi encontrada.");const ws=wb.Sheets[CORH];let removidas=0;for(let r=ref(ws).e.r;r>=5;r--)if(txt(valor(ws,r,16))===MARCADOR){removerLinha(ws,r);removidas++;}for(let r=5;r<=ref(ws).e.r;r++) [7,9,11].forEach(c=>delete ws[XLSX.utils.encode_cell({r,c})]);if(ref(ws).e.c>=16)apagarColunas(ws,14,3);return removidas; }
async function executar(operacao) {
  try { $("processar").disabled=$("resetar").disabled=true;$("status").textContent="Processando no seu navegador…";const dados=await arquivo.arrayBuffer(),wb=XLSX.read(dados,{type:"array",cellStyles:true});let resultado;if(operacao==="processar"){resultado=processar(wb);$("status").textContent=`Concluído: ${resultado.inseridas} linha(s) inserida(s), ${resultado.usados} empenho(s) usado(s), ${resultado.nao} ocorrência(s) sem correspondência.`;}else{$("status").textContent=`Reset concluído: ${resetar(wb)} linha(s) removida(s).`;}const nome=arquivo.name.replace(/\.xlsx$/i,"")+`_${operacao==='processar'?'processado':'resetado'}.xlsx`;XLSX.writeFile(wb,nome,{cellStyles:true});} catch(e) { console.error(e);$("status").textContent=`Erro: ${e.message}`;} finally{$("processar").disabled=$("resetar").disabled=!arquivo;}
}
